...
const QuestionCard = ({
  ...
  return <>
    <div className="bg-white rounded-lg mb-4 overflow-hidden py-4 px-5 shadow-xl shadow-gray-100/70 border border-gray-100/60 group">
        <div className="flex items-start justify-between cursor-pointer">
            ...
            <div className="flex items-center justify-end ml-4 relative">
                <div className={`flex ${isExpanded ? "md:flex" : "md:hidden group-hover:flex"}`}>
                    ...
                    <button className="flex items-center gap-2 text-xs text-cyan-800 font-medium bg-cyan-500 px-3 py-1 mr-2 rounded text-nowrap border border-cyan-50 hover:border-cyan-200 cursor-pointer" onClick={()=>{
                        setIsExpaned(true);
                        onLearnMore();
                    }}>
                        <LuSparkles/>
                ...
                <button className="text-gray-400 hover:text-gray-500 cursor-pointer" onClick={toggleExpand}>
                    <LuChevronDown size={20} className={`transform transition-transform duration-300 ${
        ...
        <div className="overflow-hidden transition-all durationn-300 ease-in-out" style={{maxHeight: `${height}px`}}>
                <div ref={contentRef} className="mt-4 text-gray700 bg-gray-50 px-5 py-3 rounded-lg"></div>
        </div>
    </div>
  </>
};
